...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    Y = beta0 + beta1 * X + mu + np.random.normal(0, np.sqrt(sigma2), (N, 1))
    ...
    slope = model.coef_[0][0]  # Extract the slope
    ...
    plt.figure()
    plt.hist(slopes, bins=20, alpha=0.7, color="green", label="Slopes")
    plt.hist(intercepts, bins=20, alpha=0.7, color="purple", label="Intercepts")
    plt.xlabel("Values")
...
def hypothesis_test():
    ...
    plt.figure()
    plt.hist(simulated_stats, bins=20, alpha=0.7, color="blue", label="Simulated stats")
...
def confidence_interval():
    ...
    plt.axhline(true_param, color="blue", linestyle="dashed", linewidth=2, label="True parameter")
